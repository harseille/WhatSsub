...
import { 스탭_재료_목록, 필수_선택_재료, 재료데이터_초기값 } from '@constants/constants';
...
function CustomStep(props: TProps) {
  ...
  useEffect(() => {
    const ingredients = jsonData.ingredientsData.filter(data => data.카테고리 !== '샌드위치');
    const additionalData = [
      { 카테고리: '토스팅', 목록: [{ 이름: '네' }, { 이름: '아니요' }] },
      {
        카테고리: '샌드위치',
        목록: jsonData.recipeData.map(data => ({ 이름: data.이름, 칼로리: data.재료칼로리 })),
      },
    ];

    재료데이터_수정([...ingredients, ...additionalData]);
  }, [jsonData.ingredientsData, jsonData.recipeData]);
